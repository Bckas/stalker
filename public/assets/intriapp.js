...
$("#txt_nadminPIN").keyup(function (event) {
    if (event.keyCode === 13) { changeAccessPIN(); }
});
$("#btn_nadminPIN").on("click", function () { changeAccessPIN(); });
...
$("#btn_mac").on("click", function () { save_update_mac(); });
$("#mac_sig").keyup(function (event) { if (event.keyCode === 13) { save_update_mac(); } });
$("#mac_id").keyup(function (event) { if (event.keyCode === 13) { save_update_mac(); } });
$("#mac_url").keyup(function (event) { if (event.keyCode === 13) { save_update_mac(); } });
...
$("#btn_delete_mac").on("click", function () { confirm_mac_deletion(); });
...
function confirm_mac_deletion() {
    Swal.fire({
        ...
        if (result.isConfirmed) { delete_mac_data(); }
    });
}
...
$("#toggle_proxy").on("change", function () {
    $.ajax({
        "url": "/api", "type": "POST", "data": "action=toggle_stream_proxy",
        "success": function (data) {
            try { data = JSON.parse(data); } catch (err) { }
            if (data.status == "success") { Toast.fire({ title: "Updated", text: data.message, icon: "success" }); }
            load_dashboard_data();
        }
    });
});
...
$("#toggle_logging_chk").on("change", function () {
    $.ajax({
        "url": "/api", "type": "POST", "data": "action=toggle_logging",
        "success": function (data) {
            try { data = JSON.parse(data); } catch (err) { }
            if (data.status == "success") { Toast.fire({ title: "Updated", text: data.message, icon: "success" }); }
            load_dashboard_data();
...
$("#toggle_admin_button").on("change", function () {
    $.ajax({
        "url": "/api", "type": "POST", "data": "action=toggle_admin_button",
        "success": function (data) {
            try { data = JSON.parse(data); } catch (err) { }
            if (data.status == "success") { Toast.fire({ title: "Updated", text: data.message, icon: "success" }); }
            load_dashboard_data();
...
$("#toggle_playback_cache").on("change", function () {
    $.ajax({
        "url": "/api", "type": "POST", "data": "action=toggle_playback_cache",
        "success": function (data) {
            try { data = JSON.parse(data); } catch (err) { }
            if (data.status == "success") { Toast.fire({ title: "Updated", text: data.message, icon: "success" }); }
            load_dashboard_data();
...
$("#btn_save_playback_expiry").on("click", function () {
    ...
    $.ajax({
        "url": "/api", "type": "POST", "data": "action=update_playback_expiry&expiry=" + expiry,
        "success": function (data) {
            try { data = JSON.parse(data); } catch (err) { }
            if (data.status == "success") { Toast.fire({ title: "Saved", text: data.message, icon: "success" }); }
...
$("#btn_clear_logs").on("click", function () {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/api', type: 'POST', data: 'action=clear_logs',
                success: function (response) {
                    try { response = JSON.parse(response); } catch (err) { }
                    Toast.fire('Cleared!', response.message, 'success');
                }
            });
        }
    });
...
function load_genres_for_filter() {
    $.ajax({
        url: '/api', type: 'POST', data: { action: 'get_genres' },
        success: function(response) {
...
$("#btn_save_genre_filter").on("click", function() {
    ...
    $.ajax({
        url: '/api', type: 'POST',
        data: { action: 'save_genre_filter', filter: selected },
        success: function(response) {
            try { if(typeof response === 'string') response = JSON.parse(response); } catch(e){}
