...
$copy_base_url = $streamenvproto."://".$plhoth.str_replace(" ", "%20", str_replace(basename($_SERVER['PHP_SELF']), '', $_SERVER['PHP_SELF']))."live.m3u8?id=";
?>
...
<body>
    ...
    <script>
        ...
        const copyBaseUrl = "<?php print($copy_base_url); ?>";
            ...
            batch.forEach(v => {
                ...
                html += `
                    <a href="player.php?id=${v.id}" class="card">
                        <button type="button" class="copy-btn" data-id="${v.id}" title="Copy stream URL" aria-label="Copy stream URL">
        ...
        // Copy stream URL (event delegation for lazily-rendered cards)
        grid.addEventListener('click', (e) => {
            const btn = e.target.closest('.copy-btn');
            if (!btn) return;
            e.preventDefault();
            e.stopPropagation();
            const url = copyBaseUrl + btn.dataset.id;
            const done = () => {
