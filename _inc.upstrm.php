...
function mac_handshake()
{
    ...
    $path_tokn_read = app_get_data_file_path("axToken.enc");
    $path_tokn_write = $APP_CONFIG['DATA_FOLDER']."/axToken.enc";
    if(file_exists($path_tokn_read)) {
        $ivbata = json_decode(@file_get_contents($path_tokn_read), true);
    ...
    if(empty($output['token']))
    {
        ...
        if(!empty($token)) {
            ...
            if(@file_put_contents($path_tokn_write, json_encode(array("time" => time() + 120, "token" => $token, "random" => $random), JSON_UNESCAPED_SLASHES))){}
        }
        else
        {
...
function app_macportalmeta($action)
{
    global $APP_CONFIG;
    $output = array();
    $path_Profiledt = app_get_data_file_path("axMeta.enc");
...
function mac_getGenres()
{
    ...
    $genres_path_read = app_get_data_file_path("axGenres.enc");
    $genres_path_write = $APP_CONFIG['DATA_FOLDER']."/axGenres.enc";
    
    // Check cache first
    if(file_exists($genres_path_read)) {
        $genres_data = @file_get_contents($genres_path_read);
    ...
    if(isset($adata['js']) && is_array($adata['js'])) {
        ...
        if(!empty($genres)) {
            @file_put_contents($genres_path_write, json_encode($genres, JSON_UNESCAPED_SLASHES));
...
function mac_getallChannels($retry = true)
{
    ...
    mac_getprofile();
    $output = array();
    $ctv_path_read = app_get_data_file_path("axCTV.enc");
    $ctv_path_write = $APP_CONFIG['DATA_FOLDER']."/axCTV.enc";
    
    // Check cache first
    if(file_exists($ctv_path_read)) {
        $ctv_data = @json_decode(@file_get_contents($ctv_path_read), true);
        if(isset($ctv_data[0])) { 
            return $ctv_data; 
        }
    }
    ...
    if(isset($adata['js']['data']) && !empty($adata['js']['data'])) {
        ...
        @file_put_contents($ctv_path_write, json_encode($output, JSON_UNESCAPED_SLASHES));
...
function mac_forceUpdateChannels()
{
    ...
    $ctv_path = app_get_data_file_path("axCTV.enc");
    $genres_path = app_get_data_file_path("axGenres.enc");
...
function mac_getPlaybackLink($id)
{
    ...
    $cache_path_read = app_get_data_file_path("axPBKDATA.enc");
    $cache_path_write = $APP_CONFIG['DATA_FOLDER']."/axPBKDATA.enc";
    $cache_data = array();

    if($cache_settings['status'] == "ON") {
        if(file_exists($cache_path_read)) {
            $cache_data = @json_decode(@file_get_contents($cache_path_read), true);
    ...
    if(!empty($cdetail))
    {
        ...
        if(isset($adata['js']['cmd']) && !empty($adata['js']['cmd'])) {
            ...
            if($output !== false && $cache_settings['status'] == "ON") {
                ...
                $latest_cache = array();
                if(file_exists($cache_path_read)) {
                    $l_data = @file_get_contents($cache_path_read);
                    if(!empty($l_data)) {
                ...
                @file_put_contents($cache_path_write, json_encode($latest_cache, JSON_UNESCAPED_SLASHES));
            }
        }
        else
        {
