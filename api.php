...
// Helper response function check
...
if($action == "login")
{
    ...
    if(md5($pin) == md5($irlPIN)) {
        ...
        // 2. Set Cookie (Vercel Serverless Session Fix)
        setcookie("yuvisession", "true", time() + (86400 * 30), "/");
...
elseif($action == "logout")
{
    ...
    // Clear Cookie
    setcookie("yuvisession", "", time() - 3600, "/");
...
//=====================================//
// AUTHENTICATED ACTIONS (ADMIN REQUIRED)
//=====================================//

session_start();

// Dual Validation: Session & Cookie check for Vercel Serverless
$isAuth = (isset($_SESSION['yuvisession']) && $_SESSION['yuvisession'] === true) || 
          (isset($_COOKIE['yuvisession']) && $_COOKIE['yuvisession'] === 'true');
...
{
    $path_actionlogs = app_get_data_file_path("axLogs.enc");
